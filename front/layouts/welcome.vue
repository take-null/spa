    ...
    <v-sheet>
      ...
      >
        ...
        >
          <v-col cols="12" class="text-center">
            <h1 class="display-1 mb-4 font-italic">
              {{ $config.appName }}
            </h1>
            <h6 class="subheading" :style="{ letterSpacing: '5px' }">
              読む。書く。見つける。語る。
            </h6>
          </v-col>
        </v-row>
      ...
      <v-container fluid :style="{ maxWidth: '960px' }">
        <v-row v-for="(menu, i) in menus" :key="`menu-${i}`">
          ...
          <v-col cols="12">
            <div :is="`wel-${menu.title}`" @loginPatrent="login" />
          </v-col>
        </v-row>
      </v-container>
    </v-sheet>
...
export default {
  ...
  data({ $config }) {
    return {
      params: {
        ...
        password: `${$config.guestPass}`,
      },
      imgHeight: 500,
      menus: [
        {
          title: 'about',
          subtitle: `${$config.appName}は本の情報共有サイトです`,
        },
        {
          title: 'content-one',
          subtitle: '本を検索し、書いたレビューをユーザー間で共有できます',
        },
      ],
    }
  },
  methods: {
    async login() {
      this.loading = true
      await this.$auth.loginWith('local', { data: this.params }).then((res) => {
