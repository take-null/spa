<template>
  <v-container fluid>
    <v-card flat tile color="transparent" class="mx-auto" max-width="768">
      <v-card-title> 本を探す </v-card-title>
      <v-card-text>
        本を選択してレビューを書くとタイムラインと本棚に投稿データが追加されます
      </v-card-text>
    ...
    <v-dialog v-model="postMessage" max-width="600">
    ...
    <v-dialog v-model="dialog" max-width="600">
      <v-card color="blue-grey lighten-5" class="mx-auto">
        <v-container>
          <v-row no-gutters>
            <v-col>
              <v-list-item class="d-flex flex-column justify-content-start">
                ...
                <template v-else>
                  <v-img heigth="250" width="150" :src="image" />
                </template>
              </v-list-item>
            </v-col>
            <v-col>
          ...
          <v-row>
            <v-form ref="form" v-model="isValid" class="pa-4 pt-0">
              <v-card-subtitle> 本を評価(５段階) </v-card-subtitle>
              <v-rating
                v-model="rating"
                ...
                x-small
              ></v-rating>
              <v-card-subtitle> タグを追加 </v-card-subtitle>
              <v-combobox
                v-model="model"
                :filter="filter"
                :hide-no-data="!search"
...
export default {
  ...
  methods: {
    async setBooks() {
      try {
        this.books = []
        await this.$axios
          .get(`/api/v1/books/search/?keyword=${this.keyword}`)
          .then((response) => {
            console.log('success--------------')
  ...
  data() {
    ...
    return {
      ...
      keyword: '',
      min,
      rules: [(v) => !!v || '', (v) => (!!v && min <= v.length) || false],
      isValid: false,
      loading: false,
      activator: null,
      attach: null,
