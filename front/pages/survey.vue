  ...
  <loggedInFormCard>
    ...
    >
      ...
      <v-form
        ...
        v-model="isValid"
      >
        <user-form-survey-title
          :title.sync="params.title"
        />
        <user-form-survey-content
          :content.sync="params.content"
        />
        <div>
          {{error}}
        </div>
...
export default {
  ...
  methods: {
    async sendSurvey () {
      try {
        this.loading = true
        await this.$axios.$post('/api/v1/surveys', this.params)
        .then(
        (res) => {
        this.loading = false
