<template>
  <div>
    ...
    >
      <template>
        <b-container class="d-flex justify-content-end">
          <v-btn text outlined rounded @click.stop="dialog = true">
            edit
          </v-btn>
        </b-container>
        <div v-if="image === null">
    ...
    <v-dialog v-model="dialog" fullscreen>
      <v-sheet color="blue-grey lighten-5" class="mx-auto">
        <v-container fluid :style="{ maxWidth: '768px' }">
          <template>
            <b-container class="d-flex justify-content-end">
              <v-btn text outlined rounded @click="dialog = false">
                close
              </v-btn>
            </b-container>
            <userFormImage />
            <v-form ref="form" v-model="isValid">
              <v-card-text>
                ...
                <userFormProfile :profile.sync="params.profile" />
                <userFormAge :age.sync="params.age" />
                <userFormLocate :locate.sync="params.locate" />
              </v-card-text>
