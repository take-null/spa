<template>
  ...
  >
          ...
          <v-list-item class="px-2">
            <v-list-item-avatar size="45">
              <v-btn icon outlined rounded @click.stop="toShowChild(user_id)">
                ...
                <template v-else>
                  <v-img :src="user_image" alt="avatar" />
                </template>
              </v-btn>
            </v-list-item-avatar>
    ...
    <v-container fluid mt-n2 pt-0>
      <v-row no-gutters>
        <v-col v-for="tag in tags" :key="tag.id">
          <v-btn text small @click.stop="searchTagChild(tag.name)">
            <p class="text-overline text--cyan">#{{ tag.name }}</p>
          </v-btn>
        </v-col>
      </v-row>
    </v-container>
    <v-container mt-n2 pt-0 v-if="isGood">
      <v-row dense>
        <v-col>
          <v-btn icon @click="deleteGood()">
    ...
    <v-container mt-n2 pt-0 v-else>
      <v-row dense>
        <v-col>
          <v-btn icon @click="createGood()">
...
export default {
  ...
  methods: {
    toShowChild() {
      this.$emit('toShowParent', this.user_id)
    },
    searchTagChild(tag) {
      this.$emit('searchTagParent', tag)
    },
    fetchGoodByBooksShelfId: async function () {
      const res = await this.$axios.get(
        `/api/v1/goods/?books_shelf_id=${this.id}`
      )
      if (res.status !== 200) {
      }
      return res.data
    ...
    createGood: async function () {
      const res = await this.$axios.$post('/api/v1/goods', {
        books_shelf_id: this.id,
      })
      if (res.status !== 201) {
      }
